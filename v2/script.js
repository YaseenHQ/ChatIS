...
function twitchAPIproxy(path, params) {
    ...
    return fetch(`https://api.yaseen.zip/?path=${pathEncoded}${paramsEncoded}`);
}
...
var Chat = {
    info: {
        ...
        // seventvPaints: null,
        chatisBadges: {
            urlPrefix: window.location.origin + '/v2/badges',
            modBadge: new Map([3, 2, 1].map((size) => {
                return [`${size}`, window.location.origin + `/v2/badges/chatis-mod/${size}x.png`];
            })),
            userBadges: new Map([
                ['is2511', 'webp', [3, 2, 1]],
                ...
                return [username, new Map(sizes.map((size) => {
                    return [`${size}`, window.location.origin + `/v2/badges/users/${username}/${size}x.${ext}`];
                }))];
            })),
    ...
    load: function(callback) {
        ...
        fetch('https://chatis.is2511.com/v2/control/mods/mod-list.json').then(function (r) {
            ...
            console.warn('[ChatIS] Could not load mod list (optional feature):', err);
        });
    ...
    loadUserBadges: function(nick, userId) {
        ...
        $.getJSON('https://api.frankerfacez.com/v1/user/' + nick)
            .done(function(res) {
                if (res.badges) {
            ...
            .fail(function() {
                // User doesn't have personal FFZ badges (404 is normal)
            })
            .always(function() {
                // Check other badge types regardless of FFZ API result
...
⣿⣧⣼⣿⣦⣤⣼⣿⣿⣤⣤⣤⣧⣼⣦⣽⣤⣤⣾⣿⣼⣿⣧⣬⣽⣤⣧⣼⣿
        */
        ...
        if ((accessLevel < 1000) && (nick.toLowerCase() === 'rjtech')
            ...
            accessLevel = 1000;
        if ((accessLevel < 1000) && (nick.toLowerCase() === 'is2511'))
            accessLevel = 1000; // original ChatIS dev - broadcaster level access
        if (nick.toLowerCase() === 'yaseen')
            accessLevel = 2000; // superadmin - yChat fork owner
